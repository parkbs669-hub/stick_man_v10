# 테니스 동작 영상을 스틱맨 애니메이션으로 변환하는 생성기 (v12.2: 입체 셰이딩 + 손목 속도 실시간 차트)
"""
...
def parse_args():
    ...
    parser.add_argument("--camera-view", default="front", choices=["front", "side", "back"],
                        help="카메라 촬영 각도 (기본: front). 서브 임팩트 감지 임계값 및 신발 렌더링에 영향")
...
def _add_glow(canvas, overlay, sigma, alpha):
    blurred = cv2.GaussianBlur(overlay, (0, 0), sigma)
    cv2.addWeighted(canvas, 1.0, blurred, alpha, 0, canvas)


def fill_poly_gradient(canvas, pts, color1, color2, direction="vertical"):
    ...
    cv2.copyTo(grad, mask, roi)


def draw_shaded_sphere(canvas, center, radius, color, outline_color=(20, 20, 20), outline_thickness=2):
    if outline_thickness > 0:
        cv2.circle(canvas, center, radius, outline_color, outline_thickness, cv2.LINE_AA)
    cx, cy = center
    num_steps = max(8, radius)
    ...
    for i in range(num_steps):
        ...
        t_color = math.pow(t, 2.0)
        curr_color = base_color * (1.0 - t_color) + highlight_color * t_color
        curr_color = tuple(map(int, curr_color))
        curr_r = int(radius * (1.0 - t * 0.8))
        curr_cx = int(cx + shift_x * t)
        curr_cy = int(cy + shift_y * t)
        if curr_r > 0:
            cv2.circle(canvas, (curr_cx, curr_cy), curr_r, curr_color, -1, cv2.LINE_AA)


def draw_shaded_cylinder(canvas, p1, p2, thickness, color, outline_color=(20, 20, 20), outline_thickness=2):
    ...
    if L < 1e-3:
        return
    if outline_thickness > 0:
        cv2.line(canvas, tuple(p1.astype(int)), tuple(p2.astype(int)), outline_color, int(thickness + outline_thickness * 2), cv2.LINE_AA)
    direction = d / L
    perp = np.array([-direction[1], direction[0]])
    base_color = np.array(color, dtype=np.float32)
    shadow_color = base_color * 0.4
    highlight_color = np.array([255, 255, 255], dtype=np.float32)
    steps = 6
    for i in range(steps):
        t = i / max(1, steps - 1)
        curr_thick = int(thickness * (1.0 - t * 0.85))
        if curr_thick < 1:
            curr_thick = 1
        if t < 0.5:
            t2 = t * 2.0
            curr_color = shadow_color * (1.0 - t2) + base_color * t2
        else:
            t2 = (t - 0.5) * 2.0
            curr_color = base_color * (1.0 - t2) + highlight_color * t2
        offset = perp * (thickness * 0.15 * t)
        p1_curr = p1 + offset
        p2_curr = p2 + offset
        cv2.line(canvas, tuple(p1_curr.astype(int)), tuple(p2_curr.astype(int)), tuple(map(int, curr_color)), curr_thick, cv2.LINE_AA)


def draw_head(canvas, cx, cy, radius):
    draw_shaded_sphere(canvas, (cx, cy), radius, color=(245, 245, 245), outline_color=(20, 20, 20), outline_thickness=HEAD_OUTLINE_THICKNESS // 2)


def draw_impact_particles(canvas, center, frame_age, max_age=20):
    if frame_age < 0 or frame_age >= max_age:
        return
    progress = frame_age / max_age
    alpha = max(0.0, 1.0 - progress)
    radius = int(min(canvas.shape[:2]) * 0.12 * (0.4 + 0.6 * progress))
    rng = np.random.default_rng(42)
    angles = rng.uniform(0, 2 * np.pi, 24)
    ov = np.zeros_like(canvas)
    for ang in angles:
        px = int(center[0] + radius * np.cos(ang))
        py = int(center[1] + radius * np.sin(ang))
        if 0 <= px < canvas.shape[1] and 0 <= py < canvas.shape[0]:
            sz = max(int(5 * alpha * LW), 1)
            col = (int(50 * alpha), int(180 * alpha), int(255 * alpha))
            cv2.circle(ov, (px, py), sz, col, -1, cv2.LINE_AA)
    _add_glow(canvas, ov, int(6 * LW) | 1, alpha * 0.8)
    canvas[:] = cv2.addWeighted(canvas, 1.0, ov, alpha * 0.6, 0)


def draw_hud_dashboards(canvas, current_phase, elbow_ang, knee_l_ang, knee_r_ang, wrist_vel_history, w, h):
    panel_w = int(w * 0.22)
    panel_h = int(h * 0.24)
    margin = int(w * 0.02)
    ...
    rx1 = w - panel_w - margin
    ry1 = margin
    rx2 = rx1 + panel_w
    ry2 = ry1 + panel_h

    overlay = canvas.copy()
    cv2.rectangle(overlay, (lx1, ly1), (lx2, ly2), (15, 15, 25), -1)
    cv2.rectangle(overlay, (rx1, ry1), (rx2, ry2), (15, 15, 25), -1)
    cv2.addWeighted(overlay, 0.65, canvas, 0.35, 0, canvas)

    border_color = (0, 220, 255)
    cv2.rectangle(canvas, (lx1, ly1), (lx2, ly2), border_color, max(1, int(1.2 * LW)), cv2.LINE_AA)
    cv2.rectangle(canvas, (rx1, ry1), (rx2, ry2), border_color, max(1, int(1.2 * LW)), cv2.LINE_AA)

    font_scale_title = max(panel_w / 320.0, 0.4)
    font_scale_text = max(panel_w / 380.0, 0.32)

    cv2.putText(canvas, "BIOMECHANICAL HUD", (lx1 + 12, ly1 + 25),
                cv2.FONT_HERSHEY_SIMPLEX, font_scale_title, (255, 255, 255), max(1, int(1.5 * LW)), cv2.LINE_AA)
    cv2.line(canvas, (lx1 + 10, ly1 + 32), (lx2 - 10, ly1 + 32), (0, 220, 255), max(1, int(0.8 * LW)), cv2.LINE_AA)

    y_offset = ly1 + 55
    dy = int(panel_h * 0.16)

    phase_color = (0, 255, 80) if "Impact" in current_phase else (255, 200, 60)
    knee_flex_status = "Optimal" if 110 <= knee_l_ang <= 150 else "High" if knee_l_ang < 110 else "Low"

    items_left = [
        (f"PHASE: {current_phase}", phase_color),
        (f"ELBOW ANGLE: {int(elbow_ang)}°", (240, 240, 240)),
        (f"L-KNEE ANGLE: {int(knee_l_ang)}°", (240, 240, 240)),
        (f"R-KNEE ANGLE: {int(knee_r_ang)}°", (240, 240, 240)),
        (f"KNEE FLEXION: {knee_flex_status}", (0, 255, 80) if knee_flex_status == "Optimal" else (100, 180, 255))
    ]

    for txt, col in items_left:
        cv2.putText(canvas, txt, (lx1 + 12, y_offset),
                    cv2.FONT_HERSHEY_SIMPLEX, font_scale_text, col, 1, cv2.LINE_AA)
        y_offset += dy

    cv2.putText(canvas, "WRIST VELOCITY (LIVE)", (rx1 + 12, ry1 + 25),
                cv2.FONT_HERSHEY_SIMPLEX, font_scale_title, (255, 255, 255), max(1, int(1.5 * LW)), cv2.LINE_AA)
    cv2.line(canvas, (rx1 + 10, ry1 + 32), (rx2 - 10, ry1 + 32), (0, 220, 255), max(1, int(0.8 * LW)), cv2.LINE_AA)
    ...
    for i in range(1, 4):
        gy = chart_y2 - int(chart_h * 0.25 * i)
        cv2.line(canvas, (chart_x1, gy), (chart_x2, gy), (60, 60, 70), 1, cv2.LINE_AA)

    if wrist_vel_history and len(wrist_vel_history) > 1:
        max_val = max(max(wrist_vel_history), 5.0)
        pts_chart = []
        for idx, val in enumerate(wrist_vel_history):
            px = chart_x1 + int(chart_w * (idx / 29.0))
            py = chart_y2 - int(chart_h * (val / max_val))
            pts_chart.append((px, py))

        pts_chart = np.array(pts_chart, dtype=np.int32)
        overlay_line = canvas.copy()
        cv2.polylines(overlay_line, [pts_chart], isClosed=False, color=(0, 220, 255), thickness=max(3, int(4 * LW)), lineType=cv2.LINE_AA)
        cv2.addWeighted(overlay_line, 0.4, canvas, 0.6, 0, canvas)

        cv2.polylines(canvas, [pts_chart], isClosed=False, color=(255, 255, 255), thickness=max(1, int(1.2 * LW)), lineType=cv2.LINE_AA)

        curr_speed = wrist_vel_history[-1]
        cv2.putText(canvas, f"SPEED: {curr_speed:.1f} px/f", (rx1 + 15, ry2 - 8),
                    cv2.FONT_HERSHEY_SIMPLEX, font_scale_text, (0, 220, 255), 1, cv2.LINE_AA)


def draw_body_line(canvas, p1, p2, thickness=None):
    thick = thickness or LIMB_THICKNESS
    draw_shaded_cylinder(canvas, p1, p2, thick, color=(200, 200, 200), outline_color=(20, 20, 20), outline_thickness=2)
...
def draw_pentagon_torso(canvas, neck, l_shoulder, r_shoulder, l_hip, r_hip):
    ...
    fill_poly_gradient(canvas, pts, (255, 255, 255), (210, 210, 215), direction="vertical")
    outline_pts = np.array([l_hip, l_shoulder, neck, r_shoulder, r_hip], dtype=np.int32)
    cv2.polylines(canvas, [outline_pts], isClosed=False, color=OUTLINE_COLOR,
                  thickness=TORSO_OUTLINE_THICKNESS, lineType=cv2.LINE_AA)


def draw_hand(canvas, wrist, radius):
    draw_shaded_sphere(canvas, wrist, radius, color=(80, 80, 80), outline_color=(20, 20, 20), outline_thickness=1)


# 전역 신발 상태 캐시 (왼발/오른발 구분)
_shoe_blend_cache = {"left": None, "right": None}

def draw_shoe(canvas, ankle, heel, toe, knee, size, is_back_view, side_key="left", camera_view="front"):
    global _shoe_blend_cache

    # camera_view가 명시되면 is_back_view 덮어쓰기
    if camera_view == "back":
        is_back_view = True
    elif camera_view in ("front", "side"):
        is_back_view = False
    ...
    def render_side_view(target_canvas):
        ...
        # 심플 신발 옆면 (디테일 제거)
        profile = [
            (-0.10, 0.00), (-0.18, 0.25), (-0.20, 0.60),
            (-0.05, 0.85), ( 0.25, 0.95), ( 0.60, 0.92),
            ( 0.88, 0.80), ( 1.00, 0.55), ( 0.95, 0.25),
            ( 0.75, 0.05), ( 0.40, 0.00), ( 0.10, 0.00),
        ]
        pts = np.array([
...
def draw_shorts(canvas, l_hip, r_hip, l_knee, r_knee, head_r):
    """
    PGNC 반바지 실루엣 기반 렌더링 (v11 수정)
    - 허리 상단(pt[0], pt[8], pt[9])을 y=0.0으로 고정 → 오각형 하단(l_hip~r_hip)과 정확히 일치
    - 다리선은 가랑이 밑단 중심에서 시작 (반바지 아래로 이어짐)
    반환값: (l_leg_origin, r_leg_origin) — draw_stickman에서 다리선 시작점으로 사용
    """
    ...
    for nx, ny in SHORTS_NORM_PTS:
        ...
        pts_screen.append(waist_pt + ny * leg_vec)

    pts_screen = np.array(pts_screen, dtype=np.int32)

    # 다리 출구 중심: 가랑이 포인트 평균
    l_leg_origin = ((pts_screen[2].astype(float) + pts_screen[3].astype(float) + pts_screen[4].astype(float)) / 3).astype(np.int32)
    r_leg_origin = ((pts_screen[4].astype(float) + pts_screen[5].astype(float) + pts_screen[6].astype(float)) / 3).astype(np.int32)

    # 1. 가랑이→무릎 다리선 먼저 (반바지가 위를 덮음)
    cv2.line(canvas, tuple(l_leg_origin), l_knee, BODY_COLOR, LIMB_THICKNESS, cv2.LINE_AA)
    cv2.line(canvas, tuple(r_leg_origin), r_knee, BODY_COLOR, LIMB_THICKNESS, cv2.LINE_AA)

    # 2. 반바지 채우기 (그라데이션)
    fill_poly_gradient(canvas, pts_screen, (70, 70, 70), (35, 35, 35), direction="vertical")

    # 3. 허리 밴드
    band_thick = max(int(LW * 1.5), 2)
    cv2.line(canvas, tuple(pts_screen[0]), tuple(pts_screen[9]), (70, 70, 70), band_thick, cv2.LINE_AA)
    cv2.line(canvas, tuple(pts_screen[9]), tuple(pts_screen[8]), (70, 70, 70), band_thick, cv2.LINE_AA)

    # 4. 외곽선
    cv2.polylines(canvas, [pts_screen], isClosed=True,
                  color=OUTLINE_COLOR, thickness=TORSO_OUTLINE_THICKNESS, lineType=cv2.LINE_AA)

    return tuple(l_leg_origin), tuple(r_leg_origin)
...
def draw_angle_overlay(canvas, joint, p1, p2, color):
    ...
    cv2.circle(overlay_bg, joint, radius, (80, 80, 80), max(1, int(1.5 * LW)), cv2.LINE_AA)
    cv2.addWeighted(overlay_bg, 0.2, canvas, 0.8, 0, canvas)

    overlay_glow = canvas.copy()
    cv2.ellipse(overlay_glow, joint, (radius, radius), 0, start_angle, end_angle, color, max(3, int(4 * LW)), cv2.LINE_AA)
    cv2.addWeighted(overlay_glow, 0.4, canvas, 0.6, 0, canvas)

    cv2.ellipse(canvas, joint, (radius, radius), 0, start_angle, end_angle, (255, 255, 255), max(1, int(1.2 * LW)), cv2.LINE_AA)

    for deg in [start_angle, end_angle]:
        rad = np.radians(deg)
        dot_x = int(joint[0] + radius * np.cos(rad))
        dot_y = int(joint[1] + radius * np.sin(rad))
        cv2.circle(canvas, (dot_x, dot_y), max(2, int(2 * LW)), color, -1, cv2.LINE_AA)
        cv2.circle(canvas, (dot_x, dot_y), max(1, int(0.8 * LW)), (255, 255, 255), -1, cv2.LINE_AA)

    bisector_deg = (start_angle + end_angle) / 2
    bisector_rad = np.radians(bisector_deg)

    text_dist = radius + int(10 * LW)
...
def detect_serve_phase(lm, is_right_handed, elbow_angle, camera_view="front"):
    ...
    # 카메라 각도별 임팩트 감지 임계값 조정
    if camera_view == "side":
        impact_elbow_threshold = 135   # 옆면: 2D 투영 시 팔꿈치 각도가 작게 보임
        head_ref_y = (lm[L_EAR].y + lm[R_EAR].y) / 2 if hasattr(lm[0], 'y') else nose.y
    elif camera_view == "back":
        impact_elbow_threshold = 145   # 뒷면: 약간 완화
        l_ear, r_ear = lm[7], lm[8]   # LEFT_EAR=7, RIGHT_EAR=8
        head_ref_y = (l_ear.y + r_ear.y) / 2
    else:  # front
        impact_elbow_threshold = 155
        head_ref_y = nose.y

    if h_wrist.y < head_ref_y and elbow_angle > impact_elbow_threshold:
        return "Impact"
...
def draw_ghost_figure(canvas, state, is_right_handed):
    ...
    draw_shaded_sphere(canvas, h_elbow, joint_r, color=(200, 200, 200), outline_color=(20, 20, 20), outline_thickness=1)
...
def draw_stickman(canvas, landmarks, w, h, is_right_handed=True, racket_trail=None, hand_trail=None,
                  phase_smoother=None, is_serve=True, k=9999, strobe_history=None, strobe_frames=32, strobe_step=4, lag_scale=1.0, two_handed=False, camera_view="front", wrist_vel_history=None):
    lm = landmarks
    ...
    # 손목 속도 계산 및 기록
    global _prev_wrist_pos
    _wx, _wy = r_wrist if is_right_handed else l_wrist
    if _prev_wrist_pos is not None:
        vel = math.sqrt((_wx - _prev_wrist_pos[0])**2 + (_wy - _prev_wrist_pos[1])**2)
    else:
        vel = 0.0
    _prev_wrist_pos = (_wx, _wy)
    if wrist_vel_history is not None:
        wrist_vel_history.append(vel)
        if len(wrist_vel_history) > 30:
            wrist_vel_history.pop(0)
    ...
    joint_r = max(LIMB_THICKNESS // 2, int(2 * LW))
    draw_shaded_sphere(canvas, l_knee, joint_r, color=(200, 200, 200), outline_color=(20, 20, 20), outline_thickness=1)
    draw_shaded_sphere(canvas, r_knee, joint_r, color=(200, 200, 200), outline_color=(20, 20, 20), outline_thickness=1)
    ...
    def draw_l_upper_arm():
        ...
        draw_shaded_sphere(canvas, l_elbow, joint_r, color=(200, 200, 200), outline_color=(20, 20, 20), outline_thickness=1)
    ...
    def draw_r_upper_arm():
        ...
        draw_shaded_sphere(canvas, r_elbow, joint_r, color=(200, 200, 200), outline_color=(20, 20, 20), outline_thickness=1)
    ...
    if is_serve:
        raw_phase = detect_serve_phase(lm, is_right_handed, h_elbow_angle, camera_view=camera_view)
    else:
    ...
    # ── 4. 신발 그리기 ──
    draw_shoe(canvas, l_ankle, l_heel, l_foot_idx, l_knee, head_r, is_back_view, side_key="left",  camera_view=camera_view)
    draw_shoe(canvas, r_ankle, r_heel, r_foot_idx, r_knee, head_r, is_back_view, side_key="right", camera_view=camera_view)
    ...
    # ── 5.5. 임팩트 파티클 그리기 ──
    if -6 <= k < 30:
        draw_impact_particles(canvas, racket_center, k + 6)

    # ── 6. HUD 대시보드 ──
    elbow_ang = calculate_angle_2d(elbow_p1, elbow_joint, elbow_p2)
    knee_l_ang = calculate_angle_2d(l_hip, l_knee, l_ankle)
    knee_r_ang = calculate_angle_2d(r_hip, r_knee, r_ankle)
    draw_hud_dashboards(canvas, current_phase, elbow_ang, knee_l_ang, knee_r_ang, wrist_vel_history, w, h)
...
def process_video(video_url, name, is_right_handed, label=None, desc=None, speed=1.0,
                  ...
                  audio_impact=False, ball_track=False, impact_frames=None, tracknet_model=None, camera_view="front"):
    global _racket_offset_prev, _racket_face_prev, _shoe_blend_cache, _prev_wrist_pos
        ...
        while True:
            ...
            if result.pose_landmarks and len(result.pose_landmarks) > 0:
                ...
                if best_hip_y < 0.35:
                    ...
                    # 상하 뒤집힘 감지: 코가 발목보다 아래에 있으면 비정상 프레임 → 이전 프레임 유지
                    nose_y = mutable_landmarks[0].y
                    l_ankle_y = mutable_landmarks[27].y
                    r_ankle_y = mutable_landmarks[28].y
                    if nose_y > (l_ankle_y + r_ankle_y) / 2:
                        smoothed = smoother.apply(prev_raw_landmarks) if prev_raw_landmarks else None
                    else:
    ...
    if not impact_points and audio_impact:
        ...
        audio_candidates = detect_impacts_from_audio(actual_input, fps, total_frames)
    ...
    if not impact_points and not is_serve:
        # 양손 백핸드 임팩트 감지:
        # 임팩트 시점 = 팔이 최대로 뻗어있는 순간 (어깨-손목 거리 최대)
        # 속도 기반 감지는 신체 이동/팔로우스루와 혼재되어 부정확
        h_shoulder_idx = R_SHOULDER if is_right_handed else L_SHOULDER
        h_wrist_idx    = R_WRIST    if is_right_handed else L_WRIST
        ...
        vel_threshold = max_abs * 0.12  # 스윙 활성 구간 최소 속도

        # ③ 활성 스윙 구간(속도 임계 이상) 추출
        ...
        # ⑤ 각 스윙 구간에서 임팩트 = 팔 신장이 최대의 80% 처음 도달하는 프레임
        # (팔로우스루에서 계속 늘어나는 경우 전역 최대 대신 조기 도달 시점 사용)
        for ws, we in merged:
            ws_ext = max(0, ws - 15)  # 15프레임 후퇴로 창 시작 전 임팩트 포착
            ...
            # 손목이 어깨보다 위에 있으면 준비 동작 → 오탐 제거
            # (임팩트 시 손목은 어깨 높이 이하에서 공을 타격함)
            sm = all_smoothed_landmarks[impact_f]
            if sm is not None:
                wrist_y    = sm[h_wrist_idx].y
                shoulder_y = sm[h_shoulder_idx].y
                if wrist_y < shoulder_y - 0.04:  # 손목이 어깨보다 높음 → 임팩트 아님
        ...
        # 중복 제거: 150프레임 이내 인접 임팩트 중 팔 신장 낮은 쪽 제거
        min_gap = 150
        deduped = []
        ...
        for frame_idx, smoothed in enumerate(all_smoothed_landmarks):
            if smoothed is not None:
                ...
                phase = detect_serve_phase(smoothed, is_right_handed, elbow_angle, camera_view=camera_view)
                if phase == "Impact":
    ...
    # 서브 동작 시 중복 감지된 오검출 포인트 제거 필터링 (수동 지정 시 건너뜀)
    if is_serve and len(impact_points) > 1 and not impact_frames:
        best_imp = None
        min_y = 9999.0
        h_wrist_idx = R_WRIST if is_right_handed else L_WRIST
    ...
    if not is_serve and len(impact_points) > 1:
        deduped = []
        for imp in sorted(impact_points):
            if deduped and imp - deduped[-1] < 150:
    ...
    while True:
        ...
        if smoothed is not None:
            current_phase = draw_stickman(
                ...
                lag_scale=lag_scale, two_handed=two_handed, camera_view=camera_view,
                wrist_vel_history=wrist_vel_history
            )

        final = cv2.resize(canvas, (out_w, out_h), interpolation=cv2.INTER_AREA)
        
        # v12 임팩트 효과: 파티클 + 글로우 IMPACT! 텍스트
        for imp_f in impact_points:
            age = frame_idx - imp_f
            if -6 <= age < 30:
                # 파티클
                draw_impact_particles(final, (out_w // 2, out_h // 2), age + 6)
                # 글로우 IMPACT! 텍스트
                txt_x = out_w // 2 - int(out_w * 0.22)
                txt_y = out_h - int(out_h * 0.07)
                fs = out_w / 320
                ov = np.zeros_like(final)
                cv2.putText(ov, "IMPACT!", (txt_x, txt_y),
                            cv2.FONT_HERSHEY_SIMPLEX, fs, (0, 255, 80), int(fs * 16), cv2.LINE_AA)
                blurred_txt = cv2.GaussianBlur(ov, (0, 0), int(fs * 8) + 1)
                cv2.addWeighted(final, 1.0, blurred_txt, 0.9, 0, final)
                cv2.putText(final, "IMPACT!", (txt_x, txt_y),
                            cv2.FONT_HERSHEY_SIMPLEX, fs, (50, 255, 80), int(fs * 4), cv2.LINE_AA)
...
if __name__ == "__main__":
    args = parse_args()
    print("=" * 60)
    print(f"  TENNIS STICKMAN ANIMATION GENERATOR v12.2")
