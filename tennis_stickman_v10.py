...
def parse_args():
    ...
    parser.add_argument("--bg-video", action="store_true", help="가상 코트 배경 대신 원본 비디오를 배경으로 사용")
    parser.add_argument("--real-strobe", action="store_true", help="원본 비디오 인물의 다중 노출 잔상 효과 활성화")
    parser.add_argument("--real-strobe-frames", type=int, default=20, help="실제 인물 잔상용 히스토리 프레임 수")
    parser.add_argument("--real-strobe-step", type=int, default=2, help="실제 인물 잔상 프레임 간격")
    parser.add_argument("--real-strobe-alpha", type=float, default=0.25, help="실제 인물 잔상의 기본 불투명도 스케일")
    parser.add_argument("--no-stickman", action="store_true", help="스틱맨 렌더링 비활성화 (배경 비디오 및 실제 인물 잔상만 출력)")
    parser.add_argument("--impact-offset", type=int, default=0, help="임팩트 프레임 판정 오프셋 (음수는 앞당김, 양수는 늦춤)")
...
def draw_glowing_trail(canvas, trail, color):
    ...
    # 밝은 하늘 배경에서도 궤적이 묻히지 않도록 투명도를 기존 35% -> 70%로 상향
    ...
    # 흰색 코어 비중을 기존 65% -> 30%로 낮춰 색상(노란색/파란색)이 지워지는 현상을 방지
    cv2.addWeighted(overlay_core, 0.30, canvas, 0.70, 0, canvas)
...
def process_video(video_url, name, is_right_handed, label=None, desc=None, speed=1.0,
                  ...
                  bg_video=False, real_strobe=False, real_strobe_frames=20, real_strobe_step=2, real_strobe_alpha=0.25, no_stickman=False,
                  impact_offset=0):
    ...
    if not impact_points and not is_serve:
        ...
        for ws, we in merged:
            ...
            # impact_offset 적용
            shifted_impact_f = max(0, impact_f + impact_offset)
            impact_points.append(shifted_impact_f)
            print(f"[✓] Arm-extension impact at Frame {shifted_impact_f} (originally {impact_f}, offset {impact_offset}) "
    ...
    # 비디오 초반 오검출 제거 (시작 후 1.5초 이내의 임팩트는 제외, 단 수동 지정 시에는 필터 스킵)
    if not impact_frames:
        early_cutoff = int(round(fps * 1.5))
        old_count = len(impact_points)
        impact_points = [imp for imp in impact_points if imp > early_cutoff]
        if len(impact_points) < old_count:
            print(f"[i] Filtered early impact points (before 1.5s): {impact_points}")
    ...
    real_strobe_history = [] if real_strobe else None
    ...
    while True:
        ...
        frame_resized = cv2.resize(frame, (render_w, render_h), interpolation=cv2.INTER_CUBIC)

        if real_strobe:
            canvas = frame_resized.copy()
            n_hist = len(real_strobe_history)
            if n_hist >= real_strobe_step:
                for idx in range(0, n_hist, real_strobe_step):
                    hist_frame = real_strobe_history[idx]
                    alpha = 0.05 + (real_strobe_alpha - 0.05) * (idx / max(n_hist - 1, 1))
                    cv2.addWeighted(canvas, 1.0 - alpha, hist_frame, alpha, 0, canvas)
            real_strobe_history.append(frame_resized.copy())
            if len(real_strobe_history) > real_strobe_frames:
                real_strobe_history.pop(0)
        elif bg_video:
        ...
        if not no_stickman:
            ...
            if smoothed is not None:
                # 임팩트 전후 2초에 해당하는 프레임 수 동적 계산
                strobe_limit_frames = int(round(fps * 2.0))
                # 전후 2초 이내일 때만 strobe_history 활성화
                current_strobe_history = strobe_history if (strobe_history is not None and abs(k) <= strobe_limit_frames) else None
                # 구간을 벗어나면 잔상 히스토리 버퍼를 비워 렌더링 초기화
                if strobe_history is not None and abs(k) > strobe_limit_frames:
                ...
                current_phase = draw_stickman(
                    ...
                    k=k, strobe_history=current_strobe_history, strobe_frames=strobe_frames, strobe_step=strobe_step,
...
if __name__ == "__main__":
    ...
    process_video(args.url, args.name, is_right_handed=not args.left,
                  ...
                  bg_video=args.bg_video, real_strobe=args.real_strobe,
                  real_strobe_frames=args.real_strobe_frames, real_strobe_step=args.real_strobe_step,
                  real_strobe_alpha=args.real_strobe_alpha, no_stickman=args.no_stickman,
