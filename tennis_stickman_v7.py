...
class RacketLagSimulator:
    ...
    def __init__(self, stiffness=40.0, damping=8.0):
...
# ─────────────────────────────────────────
# Step 3: Stickman Drawing & Morphing & Lag
# ─────────────────────────────────────────
...
def resample_polygon(pts, num_target_pts=16):
    pts = np.array(pts, dtype=np.float64)
    N = len(pts)
    pts_closed = np.vstack([pts, pts[0]])
    
    dists = np.sqrt(np.sum(np.diff(pts_closed, axis=0)**2, axis=1))
    cum_dists = np.zeros(N + 1)
    cum_dists[1:] = np.cumsum(dists)
    total_dist = cum_dists[-1]
    
    if total_dist == 0:
        return np.repeat(pts[:1], num_target_pts, axis=0)
        
    query_dists = np.linspace(0, total_dist, num_target_pts, endpoint=False)
    
    resampled = []
    for q in query_dists:
        idx = np.searchsorted(cum_dists, q) - 1
        idx = max(0, min(idx, N - 1))
        t = (q - cum_dists[idx]) / (cum_dists[idx+1] - cum_dists[idx] + 1e-6)
        p = (1 - t) * pts_closed[idx] + t * pts_closed[idx+1]
        resampled.append(p)
        
    return np.array(resampled, dtype=np.float64)


def draw_shoe(canvas, ankle, heel, toe, knee, size, is_back_view, foot_key):
    global _shoe_blend_prev
    ...
    pts_side = np.array([
        ...
        for fx, fy in profile_side
    ], dtype=np.float64)
    
    # ── 2. Front/Back view coordinates ──
    v_shin = ankle - knee
    d_dir = v_shin / (np.linalg.norm(v_shin) + 1e-6)
    ...
    pts_fb_raw = np.array([
        ...
        for fx, fy in profile_fb
    ], dtype=np.float64)
    pts_fb = resample_polygon(pts_fb_raw, 16)
    ...
    if L_norm <= 0.3:
        ...
        w_blend = (0.6 - L_norm) / 0.3
        
    prev_w = _shoe_blend_prev.get(foot_key, w_blend)
    alpha = 0.25
    w_smooth = alpha * w_blend + (1 - alpha) * prev_w
    _shoe_blend_prev[foot_key] = w_smooth
    
    # ── 4. Interpolate vertices ──
    pts_blended = (1 - w_smooth) * pts_side + w_smooth * pts_fb
    pts_blended = pts_blended.astype(np.int32)
    
    cv2.fillPoly(canvas, [pts_blended], SHOE_FILL_COLOR, cv2.LINE_AA)
    cv2.polylines(canvas, [pts_blended], isClosed=True, color=SHOE_OUTLINE_COLOR,
                  thickness=max(int(3 * LW), 2), lineType=cv2.LINE_AA)
                  
    # ── 5. Interpolate sole lines ──
    sole_side = np.array([
        (ankle + u * fx * length + perp * fy * height)
    ...
    sole_blended = (1 - w_smooth) * sole_side + w_smooth * sole_fb
    cv2.polylines(canvas, [sole_blended.astype(np.int32)], isClosed=False, color=(110, 110, 110),
    ...
    if w_smooth > 0:
        ...
        if is_back_view:
            ...
            cv2.line(overlay, heel_strip_start, heel_strip_end, SHOE_OUTLINE_COLOR,
                     max(int(2.5 * LW), 2), cv2.LINE_AA)
        else:
            lace_start = (ankle + d_dir * (size * 0.12))
            lace_end = (ankle + d_dir * (size * 0.45))
            cv2.line(overlay, lace_start.astype(np.int32), lace_end.astype(np.int32),
            ...
            for frac in [0.20, 0.30, 0.40]:
                ...
                cv2.line(overlay, bar_left, bar_right, (255, 255, 255),
            ...
            cv2.line(overlay, toe_cap_left, toe_cap_right, SHOE_OUTLINE_COLOR,
                     max(int(1.2 * LW), 2), cv2.LINE_AA)
                     
        cv2.addWeighted(overlay, w_smooth, canvas, 1 - w_smooth, 0, canvas)
...
def draw_racket(canvas, wrist, racket_angle, head_r):
    wx, wy = wrist
    nx = math.cos(racket_angle)
    ny = math.sin(racket_angle)
...
def draw_stickman(canvas, landmarks, w, h, is_right_handed=True, racket_trail=None, hand_trail=None, phase_smoother=None, is_serve=True, racket_simulator=None, dt=0.033):
    lm = landmarks
    ...
    # ── Racket Simulator Wrist-Lag & Trails ──
    racket_wrist = r_wrist if is_right_handed else l_wrist
    racket_elbow = r_elbow if is_right_handed else l_elbow
    wx, wy = racket_wrist
    ex, ey = racket_elbow
    target_angle = math.atan2(wy - ey, wx - ex)
    
    if racket_simulator is not None:
        racket_angle = racket_simulator.update(target_angle, dt)
    else:
        racket_angle = target_angle
    ...
    if racket_trail is not None:
        ...
        if len(racket_trail) > 20:
            racket_trail.pop(0)
        draw_glowing_trail(canvas, racket_trail, color=(0, 220, 255))
        
    if hand_trail is not None:
        hand_trail.append(racket_wrist)
        if len(hand_trail) > 20:
            hand_trail.pop(0)
        draw_glowing_trail(canvas, hand_trail, color=(255, 150, 50))
    ...
    def draw_l_forearm():
        ...
        if not is_right_handed:
            draw_racket(canvas, l_wrist, racket_angle, head_r)
    ...
    def draw_r_forearm():
        ...
        if is_right_handed:
            draw_racket(canvas, r_wrist, racket_angle, head_r)
    ...
    # 4. Shoes (with continuous shape morphing)
    draw_shoe(canvas, l_ankle, l_heel, l_foot_idx, l_knee, head_r, is_back_view, "left")
    draw_shoe(canvas, r_ankle, r_heel, r_foot_idx, r_knee, head_r, is_back_view, "right")
...
def merge_audio(video_no_audio, audio_source, output_path, speed=1.0):
    print(f"[↓] Merging audio from {audio_source} into {output_path} (speed={speed}x) ...")
    if speed == 1.0:
        cmd = [
            "ffmpeg", "-y",
            "-i", video_no_audio,
...
def process_video(video_url, name, is_right_handed, label=None, desc=None, speed=1.0):
    ...
    final_output_path = f"{name}_music.mp4"
    ...
    if not cap.isOpened():
        print(f"[✗] Cannot open video: {actual_input}")
        return
    ...
    is_serve = "serve" in name.lower() or "serve" in video_url.lower()
    ...
    # Initialize racket simulator (tuned for weaker lag: stiffness=40, damping=8)
    racket_simulator = RacketLagSimulator(stiffness=40.0, damping=8.0)
    ...
    while True:
        ...
        if result.pose_landmarks and len(result.pose_landmarks) > 0:
            ...
            current_phase = draw_stickman(canvas, smoothed, render_w, render_h, is_right_handed,
                                          ...
                                          racket_simulator=racket_simulator, dt=dt)
    ...
    print(f"\n[✓] Intermediate stickman video saved: {output_path}")
    print(f"    Resolution: {out_w}x{out_h} @ {fps:.1f}fps, {frame_idx} frames")
    
    # ── Merge Audio if original video has it ──
    if has_audio(actual_input):
        merge_success = merge_audio(output_path, actual_input, final_output_path, speed=speed)
        if merge_success:
            ...
            print(f"[!] Audio merge failed, but intermediate video is saved: {output_path}")
    else:
        print("[i] Input video has no audio stream. Final video saved without audio.")
...
if __name__ == "__main__":
    args = parse_args()
    print("=" * 60)
    print(f"  TENNIS STICKMAN ANIMATION GENERATOR v7 (Wrist-Lag + Shoe Morphing)")
