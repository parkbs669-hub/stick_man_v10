...
BODY_COLOR        = (20, 20, 20)
BODY_THICKNESS    = 12 * SSAA
HEAD_FILL_COLOR   = (255, 255, 255)
OUTLINE_COLOR     = (20, 20, 20)
OUTLINE_THICKNESS = 10 * SSAA
...
def draw_hand(canvas, wrist, radius):
    ...
    cv2.circle(canvas, wrist, radius, HAND_OUTLINE_COLOR, max(3 * SSAA, 3), cv2.LINE_AA)
...
def draw_racket(canvas, wrist, elbow, head_r):
    ...
    cv2.line(canvas, (wx, wy), (grip_end_x, grip_end_y),
             RACKET_GRIP_COLOR, max(int(8 * SSAA), 6), cv2.LINE_AA)
    ...
    cv2.ellipse(canvas, (head_cx, head_cy), (frame_rx, frame_ry),
                angle, 0, 360, RACKET_FRAME_COLOR, max(int(7 * SSAA), 5), cv2.LINE_AA)
...
def draw_stickman(canvas, landmarks, w, h, is_right_handed=True):
    ...
    head_r  = max(int(shoulder_width * 0.55), 16 * SSAA)
    head_cx = neck[0]
    head_cy = neck[1] - int(head_r * 1.05)
    hand_r  = max(int(head_r * 0.30), 6 * SSAA)
    ...
    # ── 관절 원 (Joint circles) ──
    joint_r = max(int(head_r * 0.10), 3 * SSAA)
    for jpt in [l_shoulder, r_shoulder, l_elbow, r_elbow, l_hip, r_hip, l_knee, r_knee]:
        cv2.circle(canvas, jpt, joint_r, BODY_COLOR, -1, cv2.LINE_AA)
