...
def draw_head(canvas, cx, cy, radius):
    draw_cyber_head(canvas, (cx, cy), radius)


# ── v12 사이버 렌더링 상수 ──
CYBER_BODY_COLOR   = (22, 22, 32)        # 어두운 청회색 몸체
CYBER_STRIPE_COLOR = (0, 220, 255)       # 청록 네온 스트라이프
CYBER_JOINT_COLOR  = (0, 200, 255)       # 관절 네온
CYBER_HEAD_COLOR   = (35, 35, 50)        # 헤드 어두운색
CYBER_GLOW_ALPHA   = 0.55
...
def draw_cyber_limb(canvas, p1, p2, thickness=None):
    t = thickness or LIMB_THICKNESS
    ...
    hw = t / 2.0
    pts = np.array([
        p1a + perp * hw, p2a + perp * hw,
        p2a - perp * hw, p1a - perp * hw,
    ], dtype=np.int32)
    cv2.fillPoly(canvas, [pts], CYBER_BODY_COLOR, cv2.LINE_AA)
    # 네온 스트라이프 글로우
    sw = max(int(t * 0.18), 1)
    ov = np.zeros_like(canvas)
    cv2.line(ov, tuple(p1a.astype(int)), tuple(p2a.astype(int)), CYBER_STRIPE_COLOR, sw * 2)
    _add_glow(canvas, ov, sw * 3 + 1, CYBER_GLOW_ALPHA)
    cv2.line(canvas, tuple(p1a.astype(int)), tuple(p2a.astype(int)),
             CYBER_STRIPE_COLOR, max(sw // 2, 1), cv2.LINE_AA)


def draw_cyber_joint(canvas, center, radius):
    cx, cy = int(center[0]), int(center[1])
    ov = np.zeros_like(canvas)
    cv2.circle(ov, (cx, cy), radius * 3, CYBER_JOINT_COLOR, -1)
    _add_glow(canvas, ov, radius * 2 + 1, CYBER_GLOW_ALPHA)
    cv2.circle(canvas, (cx, cy), radius, CYBER_BODY_COLOR, -1, cv2.LINE_AA)
    cv2.circle(canvas, (cx, cy), radius, CYBER_JOINT_COLOR,
               max(int(radius * 0.5), 1), cv2.LINE_AA)


def draw_cyber_head(canvas, center, radius):
    cx, cy = int(center[0]), int(center[1])
    ov = np.zeros_like(canvas)
    cv2.circle(ov, (cx, cy), int(radius * 1.4), CYBER_JOINT_COLOR, -1)
    _add_glow(canvas, ov, radius + 1, CYBER_GLOW_ALPHA * 0.6)
    cv2.circle(canvas, (cx, cy), radius, CYBER_HEAD_COLOR, -1, cv2.LINE_AA)
    cv2.circle(canvas, (cx, cy), radius, CYBER_JOINT_COLOR,
               max(int(radius * 0.12), 1), cv2.LINE_AA)
    # 하이라이트
    hx = cx - radius // 3
    hy = cy - radius // 3
    cv2.circle(canvas, (hx, hy), max(radius // 5, 2), (180, 220, 255), -1, cv2.LINE_AA)
...
def draw_impact_particles(canvas, center, frame_age, max_age=20):
    ...
    for ang in angles:
        ...
        if 0 <= px < canvas.shape[1] and 0 <= py < canvas.shape[0]:
            sz = max(int(5 * alpha), 1)
            col = (int(50 * alpha), int(180 * alpha), int(255 * alpha))
            cv2.circle(ov, (px, py), sz, col, -1, cv2.LINE_AA)
    _add_glow(canvas, ov, 6, alpha * 0.8)
    canvas[:] = cv2.addWeighted(canvas, 1.0, ov, alpha * 0.6, 0)


def draw_hud_panel(canvas, elbow_angle, knee_l, knee_r, phase, w, h):
    pw = int(w * 0.20)
    ph = int(h * 0.16)
    x1 = w - pw - int(w * 0.025)
    y1 = int(h * 0.025)
    ov = canvas.copy()
    cv2.rectangle(ov, (x1, y1), (x1 + pw, y1 + ph), (8, 8, 18), -1)
    cv2.addWeighted(ov, 0.72, canvas, 0.28, 0, canvas)
    cv2.rectangle(canvas, (x1, y1), (x1 + pw, y1 + ph),
                  CYBER_JOINT_COLOR, 1, cv2.LINE_AA)
    line_h = ph // 4
    items = [
        (f"Elbow {int(elbow_angle)}°", (0, 230, 255)),
        (f"Knee L {int(knee_l)}°",    (80, 255, 150)),
        (f"Knee R {int(knee_r)}°",    (80, 255, 150)),
        (phase,                        (255, 200, 60)),
    ]
    for i, (txt, col) in enumerate(items):
        fs = max(pw / 280, 0.28)
        cv2.putText(canvas, txt, (x1 + 5, y1 + line_h * (i + 1) - 3),
                    cv2.FONT_HERSHEY_SIMPLEX, fs, col, 1, cv2.LINE_AA)


def draw_body_line(canvas, p1, p2, thickness=None):
    draw_cyber_limb(canvas, p1, p2, thickness)
...
def draw_pentagon_torso(canvas, neck, l_shoulder, r_shoulder, l_hip, r_hip):
    ...
    # 하단 l_hip~r_hip 선 제외 — 반바지가 덮어 하얀 선 없어짐
...
def draw_shorts(canvas, l_hip, r_hip, l_knee, r_knee, head_r):
    ...
    # 2. 반바지 채우기 (차콜)
    cv2.fillPoly(canvas, [pts_screen], (45, 45, 45), cv2.LINE_AA)
...
def draw_ghost_figure(canvas, state, is_right_handed):
    ...
    draw_cyber_joint(canvas, h_elbow, joint_r)
...
def draw_stickman(canvas, landmarks, w, h, is_right_handed=True, racket_trail=None, hand_trail=None,
                  phase_smoother=None, is_serve=True, k=9999, strobe_history=None, strobe_frames=32, strobe_step=4, lag_scale=1.0, two_handed=False, camera_view="front"):
    lm = landmarks
    ...
    joint_r = max(LIMB_THICKNESS // 2, int(2 * LW))
    draw_cyber_joint(canvas, l_knee, joint_r)
    draw_cyber_joint(canvas, r_knee, joint_r)
    ...
    def draw_l_upper_arm():
        ...
        draw_cyber_joint(canvas, l_elbow, joint_r)
    ...
    def draw_r_upper_arm():
        ...
        draw_cyber_joint(canvas, r_elbow, joint_r)
    ...
    draw_hud_panel(canvas, elbow_ang, knee_l_ang, knee_r_ang, current_phase, w, h)
