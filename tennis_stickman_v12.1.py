...
import shoe_module
from shoe_module import draw_shoe, reset_cache
...
def draw_hud_dashboards(canvas, current_phase, elbow_ang, knee_l_ang, knee_r_ang, wrist_vel_history, w, h):
    ...
    elbow_status = "Extended" if elbow_ang > 140 else "Flexed"
    ...
    for txt, col in items_left:
        txt_display = txt.replace('*', '°')
        cv2.putText(canvas, txt_display, (lx1 + 12, y_offset),
...
def draw_stickman(canvas, landmarks, w, h, is_right_handed=True, racket_trail=None, hand_trail=None,
                  phase_smoother=None, is_serve=True, k=9999, strobe_history=None, strobe_frames=32, strobe_step=4,
                  lag_scale=1.0, two_handed=False, camera_view="front", wrist_vel_history=None):
    lm = landmarks
    ...
    # Calculate and record wrist velocity
    global _prev_wrist_pos
    wx, wy = r_wrist if is_right_handed else l_wrist
    if _prev_wrist_pos is not None:
        vel = math.sqrt((wx - _prev_wrist_pos[0])**2 + (wy - _prev_wrist_pos[1])**2)
    else:
        vel = 0.0
    _prev_wrist_pos = (wx, wy)
...
def process_video(video_url, name, is_right_handed, label=None, desc=None, speed=1.0,
    ...
    global _racket_offset_prev, _racket_face_prev
    # 양손 백핸드는 와이퍼 물리가 어울리지 않으므로 lag_scale 강제 0
    if two_handed:
        lag_scale = 0.0
    _racket_offset_prev = None
    _racket_face_prev = None
    shoe_module.reset_cache()
    ...
    # Reset cache and initialize wrist velocity history
    global _prev_wrist_pos
